...
if [[ ! -d $version || ! -d $version/source || `find $version -empty -type d -name source` ]]; then
   ...
   pushd $version/source &> /dev/null
      ...
      mkdir -p ../manual                                                 # |
      ns.sh - prov > ../manual/prov.csv                                                 # |
      pmap="java edu.rpi.tw.string.pmm.DefaultPrefixMappings"
      for term in `cat ../manual/prov.csv`; do 
         localname=`$pmap demo $term | grep bestLocalNameFor: | awk '{print $2}'`
         if [[ -n "$localname" ]]; then
            if [[ "$localname" == "${localname^}" ]]; then
               termtype="class"
               echo "select count(?s) where {?s a <$term>}"           > ../manual/class_$localname.rq
            else
               termtype="property"
               echo "select count(distinct ?s) where {?s <$term> []}" > ../manual/property_$localname.rq
            fi
            echo "$term ($termtype)"
         fi
      done
